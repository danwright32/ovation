...
# ASKING A REPOSITORY ABOUT ITSELF NEEDS MORE THAN `git -C`, so every question
# below goes through `clean_git` from the library, which ovation#314 moved there
# from here. An inherited GIT_DIR BEATS the -C, git EXPORTS GIT_DIR to its hooks,
# and this script's suite runs inside the pre-push hook. Found on 2026-09-08: the
# suite passed run by hand and failed five assertions inside the gate, reporting
# the pushing worktree's own branch and its 188 dirty files against a fixture repo
...
ovation_dirty_file_count() {
  local repo="$1" out
  out=$(clean_git -C "${repo}" status --porcelain 2>/dev/null) || return 0
...
ovation_provenance() {
  local repo="$1" out
  out="$(clean_git -C "${repo}" rev-parse --abbrev-ref HEAD 2>/dev/null)" || return 0
  printf '%s' "${out}"
}
...
COMMIT="$(clean_git -C "${REPO_ROOT}" rev-parse HEAD 2>/dev/null || true)"
COMMIT_DATE="$(clean_git -C "${REPO_ROOT}" log -1 --format=%cI 2>/dev/null || true)"
