...
18a. **A disagreement is expected, so it is counted rather than only handled.** Ovation keeps a running record of how often the two readers agree, per field, and Dan can see it. Leaving a field empty on a disagreement is correct, but it is also the path a systemic failure takes: one reader quietly producing nothing looks exactly like ordinary noise, one receipt at a time, and stays invisible for as long as it lasts (L77). Downbeat keeps a questionnaire agreement log for this reason and it is what turned a suspected data problem into a located parser fault. The log records counts and field names only, never the contents of a receipt.

18b. **This is measured before the rest of the expense half is built.** Both readers are run over a set of Dan's real receipts, photographed and digital, and the agreement rate is recorded per field. Neither reader's accuracy on receipts is known today, and the precedent cited for the design was measured on typed questionnaires, not on a till receipt photographed at an angle. If they rarely agree, filing an expense is typing, which is the single thing most likely to make Dan stop using this half of the product. That has to be known in the first week rather than in January.
...
8. **The on device readers are unmeasured on receipts.** Owner: Dan. Measure against real receipts before the extraction is trusted, following the approach Downbeat used for questionnaires.
