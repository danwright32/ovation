...
5a. **A one pass setup screen clears the roster before any of these guards go live**, because measured against the live export on 2026-08-28 they would fire on most invoices from day one and a warning that common is one Dan learns to click past. Of 31 clients, only 6 carry a tax status at all, so requirement 5's warning would appear on 25 of 31. One client has no contract email and two share a single contract email, so both refusals in 38 also trigger on real data immediately. The screen lists every client missing or conflicting on these fields and lets Dan settle them in one sitting. Only then do the guards start speaking, and a warning after that means something.
...
38. **A contract email is validated against what the send path can actually consume, not against a list of things it must not be** (L150, L257). Exactly one address: one `@`, no whitespace, no comma or semicolon, a dot in the domain. Anything else is refused by name, with its own reason, distinct from the empty case. Ovation never falls back to the other address and never splits a multi address value.
38a. This is not hypothetical. Measured against the live export on 2026-08-27, of 30 non empty contract email values, **two cannot be parsed as a single address**: one is 22 characters containing a space and no `@` at all, so it is human text sitting in an address field, and one is 70 characters carrying two comma separated addresses. Both would pass a check for "not empty after trimming". The second is worse than a failed send, because it puts a recipient Dan never chose onto an invoice he reviewed and approved, which breaks the rule that what a person approves must include who it goes to. Both are added as test fixtures, and the multi address one must be refused rather than split or sent to both.
