...
**Every machine checkable claim in this document is re-checkable in one command** (ovation#16): `scripts/check-plan-claims.sh`, or `--strict` to refuse on a line number that has moved. It reads every `path:line` this plan cites, every commit it names, what is actually installed, and the export's version and booking count, and it REPORTS rather than edits: a claim that has drifted may mean this document is wrong or the sibling has regressed, and only a reader can tell which. It exists because eight of these claims had already drifted by 2026-09-05 and nothing could have said so. **Since ovation#181 the push gate runs it**, so the sentence above is enforced rather than offered: on a machine holding both sibling checkouts a push is refused when this plan cites a file that is gone, one shorter than the line cited, or a path that resolves in more than one of them. Anywhere else, a runner included, it says it could not measure and refuses nothing. A line that has MOVED is still reported and not refused, because these repositories change daily and a standing red makes every other failure unreadable.
...
| **Overture's installed-build.json cannot vouch for a clean tree** | `mac/build-install.sh:140-143` | writes exactly `{version, commit, commitDate, repoPath, provenance}`. **No `dirtyFiles`, no `signingIdentity`.** A build installed from a dirty tree is recorded under a commit whose content it does not match |
...
| **A Debug Downbeat does NOT use an in-memory store** | `Downbeat/DownbeatApp.swift:55-60`; `grep -rn isDebugBuild --include=*.swift` | `isStoredInMemoryOnly: true` is gated on `AppStoreConfiguration.isRunningUnderTests()`, **never on `isDebugBuild`**. `isDebugBuild` appears in exactly two production places, both in `Downbeat/App/AppStoreConfiguration.swift`: `overtureExportURL` (line 126) and `bookingHandoffQueueURL` (line 178). **Debug isolates two file paths and nothing else** |
...
| That booking is also the one retention removes | `BookingRetention.retentionDays = 7`, `BookingSweep.run` from `Downbeat/App/LaunchMigrations.swift:2987` | 2026-08-18 is 9 days past, so the post-relaunch v3 export holds **19**, not 20 |
...
| Commit side effects | `Downbeat/Integration/Commit/CommitOrchestrator.swift:14-27, 923-940` | `Phase` = `.folder`, `.omniFocus`, `.fantastical`, `.save`, plus `rosterUpdatePhase`, `deletePredecessorPhase` and `bookingHandoffPhase` |
...
| **Downbeat's own store still uses SwiftData's default filename** | `Downbeat/App/AppStoreConfiguration.swift:97-99` | `Application Support/Downbeat/default.store`: a dedicated subfolder, but the **default filename**. Overture uses `Overture.store` |
...
| **Overture stores Gmail tokens in a file, not the Keychain** | `Overture/mac/Overture/Integration/GmailCredentials.swift:31`, `ls -la` | 0600 `gmail-tokens.json` (verified `-rw-------`), "because the app is ad-hoc signed during development" |
...
| The two siblings disagree on test locking | `Downbeat/scripts/run-tests.sh:83`, `mac/scripts/run-tests-locked.sh:22` | Downbeat: `LOCK_DIR="${TEST_LOCK_DIR:-/tmp/xcodebuild-tests.lock}"`, and its header claims the lock "is shared with Overture deliberately". Overture locks `/tmp/overture-mac-tests.lock`; `xcodebuild-tests.lock` returns **0 hits** anywhere in the Overture tree. **The sharing is not true today**, and Downbeat's lock path is overridable by `TEST_LOCK_DIR` |
| A running Debug app kills a test run | `mac/scripts/run-tests-locked.sh:99-103`, `:333-340` | a Debug app holding the single-instance lock (`LSMultipleInstancesProhibited`) makes the xctest host fail to launch; :333-340 records that a hardcoded hint naming that cause "sent hours of elimination in the wrong direction" |
...
| Downbeat is blocked on Ovation | `Downbeat/CLAUDE.md:185-187` | "#432: a crash between saving a booking and writing its queue file loses the handoff with nothing able to detect it, since Ovation deletes a file as it drains it, so absent means both consumed and never written (L258). **Needs Ovation PRD 36's consumed-id record first**" |
...
6. Write `docs/PRIVACY-FLOOR.md`. **CORRECTED 2026-09-06 (ovation#6): this step called for `docs/BEFORE-MAKING-THIS-REPO-PUBLIC.md`, ported from Downbeat, and that is the wrong document.** Dan decided on 2026-09-05 that the repository stays PUBLIC for the whole build, so a checklist to run before opening a repository that is already open enforces nothing. What is needed is a rule that holds continuously. `docs/PRIVACY-FLOOR.md` states each of the three halves and names what enforces it: `.gitignore` plus the custody check for what is committed, the identity guard for what sits in a file, and `scripts/test-output-privacy.sh` for what is PRINTED, which is step 5 above and was the half with no enforcement at all. It also states what the enforcement does NOT cover (screenshots, anything a person pastes, scripts outside the `check-*` family), and the pre private steps survive as its last section, pointing at ovation#15.
...
5. **File `downbeat#436`: the doc comment on `AppStoreConfiguration.bookingHandoffQueueURL` asserts an in-memory store under Debug that the code does not provide.** This is a comment that reads as a safety guarantee while the code provides none, in the file that decides where a record Ovation turns into a real invoice gets written. It has its own issue in `danwright32/downbeat` as `downbeat#436`, milestone `Ungrouped`, `priority-p2`, category `correctness`, filed in the same change as this step. **The number was missing here until 2026-09-10** (ovation#180): the step said `downbeat#` with nothing after it, which is a placeholder standing in for a value nobody recorded, and a placeholder is a detection that the value is absent rather than a label for it (L67). It and it is fixed by correcting the sentence (Debug isolates the export path and the queue path, and nothing else), not by relying on it.

### 0.3 The two custody snapshots: one exists, one is taken after Dan's Downbeat reinstall

**This is the genuinely unrecoverable one.** `BookingHandoffQueue.write` is called from exactly one place, `Downbeat/Integration/Commit/CommitOrchestrator.swift:937`, at commit. There is no backfill. The already-committed future bookings running to 2027-06-13 were committed under the old binary and **will never emit a queue file**, no matter when the consumer ships. `BookingRetention.retentionDays` is 7, so the `Booking` row is deleted seven days after the shoot and the recovery window closes one shoot at a time from 2026-11-01.
...
> (`Downbeat/scripts/run-tests.sh:91`), a DIRECTORY used as a mutex because `mkdir` is atomic everywhere.
...
**DECIDED 2026-09-05 by Dan: option A.** Ovation takes both existing locks, by the mechanism each one uses. Nothing in Downbeat or Overture changes. The options are kept below because the reasons for declining B and C are what stop the decision being re-taken, and the corrections underneath them are why B was priced wrongly in the first place. Tracked as ovation#12. The pre-existing Downbeat and Overture gap is a separate issue in `danwright32/overture`.

**The real options, all of which are achievable:**

- **A. Ovation takes both, by the mechanism each one uses (CHOSEN)**, in a fixed order: `mkdir` on `/tmp/xcodebuild-tests.lock` first, then `flock` on `/tmp/overture-mac-tests.lock`. Ovation then genuinely excludes both siblings, touches neither repository, and cannot deadlock because neither sibling takes two locks. **The order is fixed and stated here so it cannot drift**, because a second app taking these same two locks in the opposite order is the classic deadlock. It does not fix the pre-existing defect that Downbeat and Overture still do not exclude each other, which is filed as its own issue. It inherits Overture's Homebrew `flock` dependency, so Ovation's runner checks for it and says `brew install flock` rather than failing obscurely, copying Overture's own message at `mac/scripts/run-tests-locked.sh:516`. **Both locks are released in a trap on every exit path** (L515, L514), because a `mkdir` lock, unlike `flock`, does not self clear when its holder dies, so the trap is the whole of Ovation's crash safety on that half. Ovation ships an environment override for both paths, following Downbeat's `TEST_LOCK_DIR`, so its own runner tests never touch the real locks (L2).
- **B. Convert Overture's runner to `mkdir` on the shared directory (DECLINED)**, which would have made Downbeat's header comment true and removed the `flock` dependency from the estate entirely: one mechanism, one path, all three apps excluding each other. Declined on a cost the earlier draft had not priced. **`flock` is released by the kernel when the holding process dies. A `mkdir` lock is not**, which is exactly why Downbeat carries `scripts/claim-stale-lock.sh` and a 1800 second default `TEST_LOCK_TIMEOUT`. Converting Overture without bringing that machinery across would make Overture's runs newly vulnerable to a crash parking a stuck lock for half an hour, which is a regression traded for a tidier estate. Add to that a 1012 line script in a repository being actively worked on, the lock taken at two separate sites, and no override seam to test the change through.
- **C. Ovation takes only Downbeat's directory lock (DECLINED)**, with the full reconciliation filed for later. Simplest, and it knowingly leaves an Overture suite and an Ovation suite able to collide, which is the exact failure this step exists to prevent.

> **CORRECTED 2026-09-05**, measured against both scripts as they stand today. The mechanisms above are exactly right. Five surrounding facts were not, and they are corrected in the body rather than annotated, because a header correction standing over a contradicting body is read as the body.

| Stated before this correction | Measured 2026-09-05 |
| --- | --- |
| Downbeat's lock read at `Downbeat/scripts/run-tests.sh:83` | `:83`, and the "shared with Overture deliberately" claim is at `:11` |
| Overture's `flock` sites at `:547` and `:753` | `:612` and `:932` |
| Overture's `flock` not found check at `:470` | `:516` |
| Overture's runner is 700 lines | **1012 lines** |
| "keep the existing environment override seam" in Overture | **There is no such seam.** `LOCK_FILE` is hardcoded at `:22`. Downbeat has `TEST_LOCK_DIR`, Overture has nothing equivalent, so B would have had to build one before it could be tested at all |

**And one fact that changed the stakes.** This step assumed CI contention. Overture's self-hosted macOS runner is **gone**: `Overture/.github/workflows/ci.yml:44-47` records that #1347 removed the `swift-tests` job because the runner repeatedly went offline, leaving `runs-on: ubuntu-latest`. Mac tests run locally only, so the lock is contended by concurrent local runs and parallel agent sessions rather than by CI. That made B less dangerous than it would once have been, and it was still declined on the crash safety ground above.

**Still true and still unfixed:** `Downbeat/scripts/run-tests.sh:11` asserts the lock "is shared with Overture deliberately", and `xcodebuild-tests.lock` appears nowhere in Overture's tree. A comment that reads as a safety guarantee while providing none (L407).

**One thing any future change to Overture's runner must respect:** it must not assume one source tree per repository, because `.claude/worktrees/agent-*` holds full copies of the Overture sources and a runner or linter finding its inputs by a default recursive glob collects them (L234). **And the constraint that outlives this decision: do not ship a fourth app that locks a fifth file.** Any future Mac app in this estate takes these same two locks, or the estate is reconciled first.
...
**1.4 `Money`, Int64 minor units.** One type, no `Double` and no `Decimal` anywhere in the store or in arithmetic, enforced by `scripts/check-money-types.sh` in the push gate rather than by the type alone, which can only stop the mistake inside itself. One rounding IMPLEMENTATION, used at the two places that divide. **Corrected here in place when the code was written (ovation#53): this said the rounding rule lives at exactly one place, the tax computation. It does not.** An hourly charge at a rate that is not a whole number of dimes also lands on a fraction of a cent, and today's $250 rate is exact so nothing would have shown it. The rule is half AWAY FROM ZERO, stated for negatives because a referral credit is a negative line and rounding half toward positive infinity would make a credit round by a different amount than the charge it reverses. 8.875% of a subtotal in cents, rounded half-up, applied to the **whole subtotal** matching invoice 1057 (5.5), with 9.5 unresolved, so the rate and its base are stored on the invoice, not read from settings at render time. **Rates are frozen onto the invoice at creation**, so a settings change never silently rewrites a sent document. Property tests: no sequence of line items can make the sum of parts disagree with the stored total (5.42).
...
**1.8 Backup and restore (5.29, L7).** Dated, self-contained backups to a folder Dan chooses, requested **after** Phase 0.4's stable signing identity exists so the grant is not re-prompted on every rebuild. Rolling set. **Verification enumerates every referenced document and checks its hash**, not merely that the database opens. A backup that cannot be read back is reported as a failure, never as silence, and it reaches Dan through the one launch presenter (1.13). Restore is offered in the app and **backs up the current state before overwriting it** (L5). Rotation evicts the oldest only after the new one verifies.
   - **What a backup contains is enumerated here, not left to whoever writes it (PRD 29b):** the store and its write ahead log, the documents directory addressed by hash, both consumed-identifier ledgers, the referral ledger, the Problems store, the export run records, the `custody/` folder from 0.3, and **the Downbeat booking queue with Downbeat's `downbeat-queued-bookings.json` beside it** (ovation#253). The queue file is the only record anywhere that a shoot happened and was meant to be billed, from the day Downbeat's seven day sweep removes the booking until the drain runs. **Restore does not treat all of these alike.** Ovation's own members are replaced by the archive's copy. The queue is ADDED TO: a booking missing now is put back, and nothing already in the queue is deleted or overwritten, because a file committed after the backup is in no archive at all. While any consumed booking ledger exists, live or archived, every queued booking is held back and the restore says so, because a booking already invoiced and handed back to the drain is a second invoice to a client, and the ledger's shape is 1.11's to define. Downbeat's own file is carried and **never restored**: Downbeat rewrites it at every commit, so an older copy put back would erase every booking it recorded since, and the Phase 6 reconciliation reads it as its independent record of what was handed over. **It EXCLUDES the credential store.** A backup that lands in a folder Dan chooses, which on this Mac may sync to a Synology, must never carry a long lived refresh token granting send and modify on his mailbox. **A guard asserts no backup archive contains the token file**, by name and by content hash, and it is seen to fail by planting the token file in the staging set before it is trusted (L1, L19). A restore that does not require re-authorising Gmail is its own decision with its own sign off, never a side effect of being thorough.
...
**1.11 Consumed-booking ledger (6.36, L512), and the cross-repo deliverable it closes.** Ovation's own durable append-only record of every booking id it has consumed, independent of the queue files it deletes, so a booking whose invoice is later lost is recoverable after Downbeat's copy is swept. A forward-only drain's first gap is permanent and the redo path is invisible until a gap exists. Same shape reused for consumed Gmail message ids in Phase 4.
   - **This is not an internal Ovation concern. `downbeat#432` is parked waiting on it**, per `Downbeat/CLAUDE.md:185-187`: a crash between saving a booking and writing its queue file loses the handoff with nothing able to detect it, because Ovation acknowledges by deleting, so absent means both "consumed" and "never written" (L258).
...
- The full domain model on top of Phase 1's types: `Client`, `Invoice`, `LineItem`, `Payment`, `Refund`, `Expense`, `ExpenseCategory`, `ReferralLedgerEntry`, `ServiceType`, and TWO the plan's list was short: **`Shoot`**, because PRD 5.1a made an invoice able to cover more than one, and **`PaymentAllocation`**, because PRD 5.14 made a payment its own record split across invoices. Both were added on 2026-09-06, after this list was written. There is deliberately **no `Booking` entity**: PRD 5.1b's three endings are all states of an Invoice, since 5.1c settles that dismissing happens from inside the invoice and keeps its row, and the handoff side is the consumed booking ledger in ovation#31. **Corrected in place 2026-09-07 (ovation#60): `@Attribute(.unique)` appears NOWHERE, not even on the surrogate `id: UUID`.** This line previously said unique on the surrogate id only, which was read off Downbeat's usage rather than measured, and PRD 42a's own finding says the attribute is what destroys the original row. `SwiftDataBehaviourTests` measures both halves: with the attribute a collision leaves one row holding the second value, without it the same collision leaves two rows with the original intact. See PRD 42c.
...
**Corrected 2026-09-11, and most of the paragraph above is now history rather than a plan.** Dan was shown the unparseable value and fixed it in Downbeat, at source, so the live export measures ZERO clients whose address cannot be sent to. Two of the three shapes that paragraph describes were already ruled not to be faults on 2026-09-07 (PRD 37c, 38a, 38c), so what is left for the roster pass to draw is the 25 missing tax statuses, and that half is cleared in Ovation because Ovation owns the field once Dan answers it (PRD 5a). **Corrected 2026-09-11 (ovation#215): this sentence said the export has no field for tax status at all, which was false.** It carries `isTaxExempt`, and the 25 is exactly the count of clients where it is absent, so ovation#208 imports the 6 answered values and the pass clears the rest. **The refusal, its fixture and its section on the screen all stay**, because a client typed into Ovation by hand can still carry a value that is not an address; what changes is that the section is not DRAWN while it is empty (PRD 5a). **Nothing in Ovation edits an address, and nothing needs to**, which is the question the roster screen could not answer for itself: both it and the Clients detail pane draw a bad address in a read only field, so before this the pass showed a blocker it offered no way to clear. Re-derive every figure here with `scripts/measure-booking-export.py` against the current export rather than reading it off this page (L175).
...
| The doc comment on `AppStoreConfiguration.bookingHandoffQueueURL` asserts a throwaway in-memory store under Debug that the code does not provide | `danwright32/downbeat` | 0.2 step 5. `Downbeat/DownbeatApp.swift:55-60` gates in-memory on tests, never on Debug |
