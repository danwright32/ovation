...
| --- | --- |
| Downbeat's lock read at `scripts/run-tests.sh:50` | `:83`, and the "shared with Overture deliberately" claim is at `:11` |
...
**And one fact that changed the stakes.** This step assumed CI contention. Overture's self-hosted macOS runner is **gone**: `ci.yml:44-47` records that #1347 removed the `swift-tests` job because the runner repeatedly went offline, leaving `runs-on: ubuntu-latest`. Mac tests run locally only, so the lock is contended by concurrent local runs and parallel agent sessions rather than by CI. That made B less dangerous than it would once have been, and it was still declined on the crash safety ground above.

**Still true and still unfixed:** `scripts/run-tests.sh:11` asserts the lock "is shared with Overture deliberately", and `xcodebuild-tests.lock` appears nowhere in Overture's tree. A comment that reads as a safety guarantee while providing none (L407).
