...
| The foot's three sentences | Payment instructions, the note and the contact come from Settings rather than from the source (ovation#319, PRD 9), so what a client reads is what Dan typed. A block with nothing in it is left off the page rather than drawn as a heading over a gap; an empty payment line or contact stops every send, each naming Settings rather than the invoice in front of him; the note is optional. The wording was read cold on the rendered page on 2026-09-17 (PRD 41a). The note became `delivered within two weeks after the shoot`, because `five business days` promised a delivery from no stated starting point and was already untrue on any invoice sent a fortnight after the performance. `Payment instructions available upon request.` was read in place and kept as it stands (Dan, 2026-09-17), and is now his to change without a code change. |
